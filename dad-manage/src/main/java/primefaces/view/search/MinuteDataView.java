...
@ManagedBean
@ViewScoped
public class MinuteDataView {
	
	private final static Logger log = LoggerFactory.getLogger(MinuteDataView.class);
	...
	private Date time;
	private List<PollutantsCountData> minuteDatas;
	...
	public void refreshData() {
		...
		if(StringUtils.isEmpty(deviceId)) {
			...
			message = new FacesMessage(FacesMessage.SEVERITY_WARN, "验证失败","时间不能为空");
		} else {
			try {
				minuteDatas = dadDataService.getMinuteDatas(deviceId, dataCode, DateUtil.getStringByDate(time, DateUtil.hourFmt));
			} catch (Exception e) {
				log.error(e.getMessage(), e);
	...
	public List<PollutantsCountData> getMinuteDatas() {
		return minuteDatas;
	}
