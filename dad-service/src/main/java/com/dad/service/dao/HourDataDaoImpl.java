...
public class HourDataDaoImpl implements HourDataDao {
	...
	public static Logger log = LoggerFactory.getLogger(HourDataDaoImpl.class);
	...
	private static String getHourDataSql = "SELECT device_id,data_code,data_time,cou,min,max,avg,zs_min,zs_max,zs_avg FROM %s WHERE device_id=? AND data_code=? AND data_time BETWEEN ? AND ? ORDER BY data_time ASC";

	public void saveHourDatas(final List<PollutantsCountData> counts,
			String dataTime) {
		TableEntity table = TableRouter.routerTable(dataTime,
				TableRouter.hourTable);
		if(table == null) {
		...
		dynamicTemplate.batchUpdate(
				String.format(insertSql, table.getTableName()),
				new BatchPreparedStatementSetter() {
					...
					public void setValues(PreparedStatement ps, int i)
						...
						ps.setTimestamp(3, new Timestamp(data.getDataTime()
								.getTime()));
						if (data.getCou() == null) {
	...
	@Override
	public List<PollutantsCountData> getHourDatas(final String deviceId,
			final String dataCode, final Date start, final Date end, final String dayTime) {
		TableEntity table = TableRouter.routerTable(dayTime,
				TableRouter.hourTable);
		if (table == null) {
			log.error("{} no table router!", dayTime);
			return null;
		}
		DataSourceUtil.setDataSourceKey(table.getDbKey());
		return dynamicTemplate.query(
				String.format(getHourDataSql, table.getTableName()),
