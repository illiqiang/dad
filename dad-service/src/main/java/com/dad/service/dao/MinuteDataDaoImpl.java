...
public class MinuteDataDaoImpl implements MinuteDataDao {

	public static Logger log = LoggerFactory.getLogger(MinuteDataDaoImpl.class);
	...
	public void saveMinuteDatas(final List<PollutantsCountData> counts,
		...
		TableEntity table = TableRouter.routerTable(dataTime,
				TableRouter.minuteTable);
		if (table == null) {
			unknowdblog.info("saveMinuteDatas({})", counts);
	...
	@Override
	public List<PollutantsCountData> getMinuteDatas(final String deviceId,
			final String dataCode, final Date start, final Date end,
			String hourTime) {
		TableEntity table = TableRouter.routerTable(hourTime,
				TableRouter.minuteTable);
		if (table == null) {
			log.error("{} no table router!", hourTime);
			return null;
		}
		DataSourceUtil.setDataSourceKey(table.getDbKey());
		return dynamicTemplate.query(
				String.format(getMinuteDataSql, table.getTableName()),
